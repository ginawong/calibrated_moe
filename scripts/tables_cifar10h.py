#!/usr/bin/env python
"""CIFAR-10H results table (LaTeX, plain text, HTML, PNG).

Reads per-(method, seed) JSON metrics from <results_dir>/<method>_s<seed>/*.json
and aggregates across seeds for the 7 paper methods.

Also produces reliability diagrams and confidence-vs-agreement plots; the
canonical sources for those are scripts/figures_reliability.py and
scripts/figures_titleless.py, but they are convenient to have alongside the table.

Usage:
    python scripts/tables_cifar10h.py --output_dir output/cifar10h
"""
...
DEFAULT_RESULTS_DIR = 'experiment_results/cifar10h/results'
DEFAULT_OUTPUT_DIR = 'output/cifar10h'

# Paper-facing method order; aggregation only includes these.
ALL_METHODS = [
...
def load_all_results(results_dir):
    ...
    Sweep variants (e.g. `robust_filtered_dt0.01_s42/`) are deliberately ignored;
    the script aggregates only the canonical-named runs that define the headline
    table. Set --results_dir to point at a different layout.
    """
    results = []
    for method in ALL_METHODS:
        for path in glob.glob(f'{results_dir}/{method}_s*/*.json'):
            # Validate that the parent dir matches `{method}_s{int}` exactly.
            ...
            if 'base' not in data:
                continue
            # The "method" field on disk reflects what was trained.
...
def _is_canonical_run_dir(dirname, method):
    """True iff dirname is exactly `{method}_s{integer}`."""
    prefix = f'{method}_s'
...
def aggregate(all_results):
    """Aggregate canonical results by method across seeds."""
...
def get_predictions(model, loader, device, is_moe=True, per_expert_temps=None):
...
def load_all_predictions(results_dir, device):
    ...
    for method in ALL_METHODS:
        ...
        for seed in SEEDS:
            ...
            method_preds.append(get_predictions(model, loader, device, is_moe=is_moe,
    ...
    return all_predictions, temperatures, agreement
...
def plot_table(agg, output_dir):
    """Render the results table as a figure."""
    columns = ['Method', 'Acc', 'Hard Acc', 'ECE', 'ECE+TS', 'Hard ECE', 'Hard ECE+TS']
    col_keys = [
        ('base_accuracy', None),
        ('base_hard_acc', None),
        ('base_ece', None),
        ('temp_scaled_ece', None),
        ('base_hard_ece', None),
        ('temp_scaled_hard_ece', None),
    ]
    ...
    fig, ax = plt.subplots(figsize=(14, 0.5 * len(rows) + 1.8))
    ax.axis('off')
    ...
    for i, method in enumerate([m for m in ALL_METHODS if m in agg]):
        cell = table[i + 1, 0]
        cell.set_text_props(color=COLORS[method], fontweight='bold')

    # Bold best values per column (lower is better for ECE, higher for Acc)
    higher_better = [True, True, False, False, False, False]  # columns 1-6
    for col_idx in range(6):
        ...
        for row_idx, v in enumerate(vals):
            if v is not None and abs(v - best) < 1e-4:
                cell = table[row_idx + 1, col_idx + 1]
                cell.set_text_props(fontweight='bold')

    ax.set_title('CIFAR-10H: All Methods (5-trial mean \u00b1 std)', fontsize=14,
                 fontweight='bold', pad=20)
    plt.tight_layout()
    _savefig(os.path.join(output_dir, 'cifar10h_table.png'))
    ...
    txt_path = os.path.join(output_dir, 'cifar10h_table.txt')
    ...
    html_lines = [
        '<table>',
        '<caption>CIFAR-10H: All Methods (5-trial mean &pm; std)</caption>',
        '<thead>',
    ...
    html_path = os.path.join(output_dir, 'cifar10h_table.html')
    ...
    tex_path = os.path.join(output_dir, 'cifar10h_table.tex')
...
# ==========================================
# Figure 2 & 3: Reliability diagrams
# ==========================================
...
def plot_reliability_diagrams(all_predictions, agreement, output_dir, subset='all',
    ...
    subset: 'all' for all images, 'hard' for agreement < 0.7.
    """
    ...
    if subset == 'hard':
        mask = agreement < 0.7
        title_suffix = f'Hard Images (agreement < 0.7, n={int(mask.sum())})'
    else:
        mask = np.ones(len(agreement), dtype=bool)
        title_suffix = f'All Images (n={int(mask.sum())})'
    ...
    fig.suptitle(f'CIFAR-10H Reliability{ts_suffix} \u2014 {title_suffix}, {n_seeds}-trial mean \u00b1 std',
                 fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    suffix = 'hard' if subset == 'hard' else 'all'
    ts_tag = '_ts' if temp_scaled else ''
    _savefig(os.path.join(output_dir, f'cifar10h_reliability_{suffix}{ts_tag}.png'))


# ==========================================
# Figure 4: Confidence vs human agreement
# ==========================================
def plot_confidence_vs_agreement(predictions, agreement, output_dir):
    available = [m for m in ALL_METHODS if m in predictions]
    ...
    ax.text(1.0, 0.03, 'Hard region', fontsize=13, color='red',
            ha='center', fontweight='bold', alpha=0.7)

    ax.set_xlabel('Human Agreement Level', fontsize=14)
    ax.set_ylabel('Mean Confidence', fontsize=14)
    ax.set_title('CIFAR-10H: Confidence vs Human Agreement', fontsize=16, fontweight='bold')
    ...
    _savefig(os.path.join(output_dir, 'cifar10h_confidence_vs_agreement.png'))
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description='Generate CIFAR-10H results table.')
    ...
    parser.add_argument('--table_only', action='store_true',
                        help='Generate the .tex/.txt/.html/.png table only '
                             '(skip the reliability and confidence-vs-agreement figures).')
    ...
    all_predictions, temperatures, agreement = load_all_predictions(args.results_dir, args.device)

    print("\nReliability diagrams (all images)")
    plot_reliability_diagrams(all_predictions, agreement, out, subset='all')
    print("\nReliability diagrams (all images, temp scaled)")
    plot_reliability_diagrams(all_predictions, agreement, out, subset='all',
                              temperatures=temperatures, temp_scaled=True)
    print("\nReliability diagrams (hard images)")
    plot_reliability_diagrams(all_predictions, agreement, out, subset='hard')
    print("\nReliability diagrams (hard images, temp scaled)")
    plot_reliability_diagrams(all_predictions, agreement, out, subset='hard',
                              temperatures=temperatures, temp_scaled=True)

    predictions_s42 = {m: preds[0] for m, preds in all_predictions.items()}
    print("\nConfidence vs human agreement")
    plot_confidence_vs_agreement(predictions_s42, agreement, out)

    print(f"\nAll CIFAR-10H outputs saved under {out}/")
